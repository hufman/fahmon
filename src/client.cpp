...
void Client::Reload(void)
{
    ...
    {
        mLog = wxString::Format(wxT("Directory %s does not exist or cannot be read!"), templocation.c_str());
        _LogMsgError(mLog);
        return;
    }
    else
    ...
    {
        mLog = wxString::Format(wxT("Error while reading %sFAHlog.txt!"), templocation.c_str());
        _LogMsgError(mLog);
        mState = ST_INACCESSIBLE;
        return;
    }
    ...
    if(!LoadUnitInfoFile(templocation + wxT("unitinfo.txt")))
        _LogMsgError(wxString::Format(wxT("Error while reading %sunitinfo.txt!"), templocation.c_str()));

    // Try to load the configuration file 'client.cfg'
    if(!LoadClientCfg(templocation + wxT("client.cfg")))
        _LogMsgError(wxString::Format(wxT("Error while reading %sclient.cfg!"), templocation.c_str()));
...
inline bool Client::LoadUnitInfoFile(const wxString& filename)
{
	...
	*
    ...
    if(!projectOk)
        _LogMsgWarning(wxString::Format(wxT("No valid project number was found for %s in FAHlog.txt"), mName.c_str()));
...
void Client::ComputeETA(WorkUnitFrame* lastFrame)
{
    ...
    if(lastFrame != NULL)
    {
        _LogMsgInfo(wxString::Format(wxT("%s is on frame %u"), mName.c_str(), lastFrame->GetId()));
    }
    else
    {
        _LogMsgInfo(wxString::Format(wxT("Cannot determine frame number for %s (this isn't a problem)"), mName.c_str()));
    }
...
void Client::FindCurrentState(WorkUnitFrame* lastFrame)
{
    ...
    else
    {
        ...
        {
            mState = ST_HUNG;
            _LogMsgWarning(wxString::Format(wxT("%s seems to have hung : Elapsed time is %um and limit is %um"), mName.c_str(),   lastFrame->GetElapsedSeconds()/60, trigger/30));
        }
        else
        {
            mState = ST_INACTIVE;
            _LogMsgInfo(wxString::Format(wxT("%s seems to be inactive : Elapsed time is %um and limit is %um"), mName.c_str(),   lastFrame->GetElapsedSeconds()/60, trigger/60));
        }
    }
}
