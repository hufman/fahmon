...
void Client::Reload(void)
{
	...
	if(!LoadWULogFile(wxString::Format(wxT("%swork/logfile_%02i.txt"), templocation.c_str(), mUnitIndex)))
	{
		_LogMsgError(wxString::Format(_("Error while reading %swork/logfile_%02i.txt"), templocation.c_str(), mUnitIndex));
	}
...
bool Client::LoadWULogFile(wxString const &filename)
{
	...
	versionOk     = false;
	startingPos = currentLine.Find(wxT("Version")) + 8;
	endingPos = currentLine.Find('(');
